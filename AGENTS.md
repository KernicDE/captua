...
- **Fixed window size**: `compute_window_size()` (overlay.py) sizes the window once to content + 50px margin + toolbar height, capped at the available screen space (`availableGeometry()` minus 40px). The pill toolbar keeps its natural width (recomputed on property-panel toggle) and the `ToolbarScrollArea` shows a slim horizontal scrollbar when the window is narrower. `scene_rect_fitted` no longer resizes the window; overflowing content stays reachable via pan/zoom. After `showEvent`, the size is re-asserted twice via `QTimer` (`_reassert_size`) because compositors may impose a default height on new floating windows (e.g. a global niri `default-window-height` rule).
