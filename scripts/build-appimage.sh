...
if [ -d "${PYSIDE6_DIR}" ]; then
    ...
    for tool in designer linguist lrelease lupdate qml qmlcachegen qmlimportscanner \
                ...
                qt-configure-module qtpaths qtplugininfo pixeltool qsb; do
    ...
    if [ -d "${PYSIDE6_DIR}/Qt/lib" ]; then
        for f in "${PYSIDE6_DIR}/Qt/lib"/*.so*; do
            ...
            basename=$(basename "${f}")
            case "${basename}" in
                libQt6Core.so*|libQt6Gui.so*|libQt6Widgets.so*|libQt6DBus.so*|libQt6OpenGL.so*|libQt6WaylandClient.so*|libQt6WlShellIntegration.so*|libQt6Svg.so*|libQt6XcbQpa.so*|libicui18n.so*|libicuuc.so*|libicudata.so*)
                    ;;
                *)
                    rm -f "${f}"
                    ;;
