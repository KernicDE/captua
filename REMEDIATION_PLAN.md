...
|----|-----------|-------------------|-----------|---------|
| R-01 | Korrektheit | **`History.clear()` fehlt.** `CropTool.mouse_release()` ruft `self.history.clear()` auf, die Methode existiert in `history.py` nicht. Jeder Crop-Vorgang wirft `AttributeError`. → Methode in `History` ergänzen, die `_undo` und `_redo` leert. | P0 | S |
| R-02 | Sicherheit | **Orphaned Temp-Dateien mit Screenshot-Inhalt.** `BlurTool._blur_region()` und `MagnifierTool._zoom_region()` erzeugen jeweils zwei `NamedTemporaryFile`-Dateien (`in_path`, `out_path`) ohne `try/finally`-Cleanup. Bei jeder Exception (Pillow-Fehler, Speichermangel) verbleiben Pixeldaten des Screenshots in `/tmp`. → Beide Pfade in einen `try/finally`-Block aufnehmen; `Path(p).unlink(missing_ok=True)` im `finally`-Zweig für jede Datei. | P0 | S |
| R-03 | Codequalität | **Doppelte Imports in Funktionsrümpfen.** `copy_pixmap_to_clipboard()` importiert `tempfile` und `pathlib.Path` erneut innerhalb der Funktion, obwohl `tempfile` bereits auf Modulebene importiert ist. `capture_window()` importiert `json` innerhalb des Funktionsrumpfes. → Alle Imports an den Modulanfang verschieben. | P0 | S |
| R-04 | Performance | **Disk-Roundtrip in Blur- und Magnifier-Pipeline.** Beide Tools rendern eine QPixmap, speichern sie als PNG nach `/tmp`, öffnen die Datei mit Pillow, verarbeiten sie und speichern erneut als PNG, bevor sie als QPixmap geladen wird. Bei 4K-Regionen: zwei synchrone Schreibvorgänge à ≥ 25 MB je Aufruf, plus Pillow-Encode-/Decode-Overhead. → `QPixmap` via `QBuffer`/`QByteArray` in einen In-Memory-PNG-Stream konvertieren; Pillow mit `Image.open(BytesIO(...))` laden; Rückkonvertierung analog. Kein Dateisystem-I/O. | P1 | M |
...
| R-09 | UX/Persistenz | **Toolbar-Änderungen (Farbe, Linienstärke) werden bei Fensterschluss verworfen.** Settings werden nur beim OK-Klick im Backdrop-Dialog persistiert (`_show_backdrop_dialog`). Farbänderungen via Toolbar landen nie auf Disk. → `save_settings()` im `closeEvent` von `OverlayWindow` aufrufen; `extract_from_scene()` und alle `_props`-Felder (line_color, line_width, fill_color, fill_alpha) dort bündeln. | P1 | S |
| R-10 | Sicherheit | **`save_settings()` ist nicht atomisch.** `json.dump()` schreibt direkt in die Zieldatei. Ein Absturz oder `KeyboardInterrupt` während des Schreibens hinterlässt eine korrupte `settings.json`. → In eine temporäre Datei im selben Verzeichnis schreiben, dann `Path.replace()` (atomisches Rename auf POSIX) aufrufen. | P1 | S |
| R-11 | Kodequalität | **Broad `except Exception: return None` in `_blur_region` und `_zoom_region`.** Alle Fehler (Pillow, Speicher, Qt-Fehler) werden stillschweigend verschluckt. Der Nutzer sieht kein Feedback; Debugging ist unmöglich. → Zumindest `logging.exception()` aufrufen; spezifische Exceptions (`OSError`, `PIL.UnidentifiedImageError`) statt der Catch-All-Klausel verwenden. | P1 | S |
...
| R-17 | Sicherheit | **Kein Zeitlimit für Subprozesse.** `_run()` und die direkten `subprocess.run()`-Aufrufe setzen kein `timeout`-Argument. Ein hängender `slurp`- oder `grim`-Prozess blockiert den Main Thread dauerhaft. → `timeout=15` (oder konfigurierbar) in alle `subprocess.run()`-Aufrufe einbauen; `subprocess.TimeoutExpired` als eigene Exception behandeln. | P2 | S |
